...
async function onDisconnect(info) {
  ...
  if (window.DaVinci.wallet.isOneWallet) {
    window.DaVinci.wallet.forgetIdentity()
  }
...
async function onChain(chainId) {
  ...
  if (chainId == window.DaVinci.chainId) {
    ...
    if (wallet == 'metamask') {
      if (window.ethereum && window.ethereum.isMetaMask) {
        ...
        this.wallet.getAccount = async function () {
          ...
          if (accts.length > 0) {
            return { address: accts[0].toLowerCase() }
          } else {
            return { address: null }
          }
        }
      ...
      if (window.onewallet && window.onewallet.isOneWallet) {
        ...
        this.wallet.isConnected = function () {
...
*/
  ...
  async waitForReceipt(hash, n = 0) {
    try {
      ...
      console.log('Error in receipt', ex)
    }
    return null
  }
